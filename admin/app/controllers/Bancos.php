...
if(!empty($_POST['banco']) && empty($_POST['validarData']) && empty($_GET['operation']) ){
  ...
  $query = "INSERT INTO bancos (id_banco, nombre_banco, codigo_banco, opcion_pago, tipo_cuenta, numero_cuenta, nombre_propietario, cedula_cuenta, telefono_cuenta, estatus) VALUES (DEFAULT, '{$bancos}', '{$cod_banco}', '{$opcion_pago}', '{$tipo_cuenta}', '{$numero_cuenta}', '{$nombre_propietario}', '{$cedula_cuenta}', '{$telefono_cuenta}', 1)";
...
if(!empty($_POST['banco']) && empty($_POST['validarData']) && !empty($_GET['operation']) && $_GET['operation'] == "Modificar"){
  ...
  $query = "UPDATE bancos SET nombre_banco = '$banco', codigo_banco='{$cod_banco}', opcion_pago='{$opcion_pago}', tipo_cuenta='{$tipo_cuenta}', numero_cuenta='{$numero_cuenta}', nombre_propietario='{$nombre_propietario}', cedula_cuenta='{$cedula_cuenta}', telefono_cuenta='{$telefono_cuenta}', estatus = 1 WHERE id_banco = $id";
