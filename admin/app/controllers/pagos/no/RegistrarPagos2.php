		...
		if(!empty($_POST['valForma']) && !empty($_POST['fechaPago']) && !empty($_POST['tipoPago']) ){
			...
			if($id_banco != ""){
				...
				if($banco['nombre_banco']=="Provincial"){
					...
					if($referencia!=""){
						// $buscar = $lider->consultarQuery("SELECT * FROM pagos WHERE referencia_pago = '{$referencia}' and estatus = 1");
						// $buscar = $lider->consultarQuery("SELECT * FROM pagos WHERE referencia_pago = '{$referencia}' and id_campana = {$id_campana}");
			...
			if(Count($buscar)<2){
				if($id_banco != ""){
					...
					if($banco['nombre_banco']=="Provincial"){
						...
						if($referencia!=""){
							...
							// $buscar = $lider->consultarQuery("SELECT * FROM pagos WHERE fecha_pago = '{$fechaPago}' and referencia_pago = '{$referencia}' and monto_pago = '{$monto}' and  id_campana = {$id_campana}");
				...
				if(Count($buscar)<2){
					...
					if($id_banco==""){
						// if($forma_pago=="Divisas Dolares"){
						// 	$query = "INSERT INTO pagos (id_pago, id_pedido, id_campana, fecha_pago, fecha_registro, forma_pago, tipo_pago, referencia_pago, monto_pago, tasa_pago, equivalente_pago, estatus) VALUES (DEFAULT, $id_pedido, $id_campana, '$fechaPago', '".date('Y-m-d')."', '$forma_pago', '$tipoPago', '$serial', '$monto', '$tasa', '$eqv', 1)";
						// }
						// if($forma_pago=="Divisas Euros"){
						// 	$query = "INSERT INTO pagos (id_pago, id_pedido, id_campana, fecha_pago, fecha_registro, forma_pago, tipo_pago, referencia_pago, monto_pago, tasa_pago, equivalente_pago, estatus) VALUES (DEFAULT, $id_pedido, $id_campana, '$fechaPago', '".date('Y-m-d')."', '$forma_pago', '$tipoPago', '$serial', '$monto', '$tasa', '$eqv', 1)";
						// }
						...
						if($referencia!=""){
							...
							$query = "INSERT INTO pagos (id_pago, id_pedido, id_campana, id_banco, fecha_pago, fecha_registro, forma_pago, tipo_pago, referencia_pago, monto_pago, tasa_pago, equivalente_pago, estatus) VALUES (DEFAULT, $id_pedido, $id_campana, $id_banco, '$fechaPago', '".date('Y-m-d')."', '$forma_pago', '$tipoPago', '$referencia', '$monto', '$tasa', '$eqv', 1)";
						}else if($serial!=""){
							// echo "Hay Serial no Referencia";
							$query = "INSERT INTO pagos (id_pago, id_pedido, id_campana, id_banco, fecha_pago, fecha_registro, forma_pago, tipo_pago, referencia_pago, monto_pago, tasa_pago, equivalente_pago, estatus) VALUES (DEFAULT, $id_pedido, $id_campana, $id_banco, '$fechaPago', '".date('Y-m-d')."', '$forma_pago', '$tipoPago', '$serial', '$monto', '$tasa', '$eqv', 1)";
						}
					}
