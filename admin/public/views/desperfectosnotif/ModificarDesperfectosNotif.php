                  ...
                  <div class="row">
                    ...
                    <?php foreach ($colecciones as $collss): if(!empty($collss['id_producto'])):?>
                      ...
                      <div class="col-xs-12 col-md-4">
                        <input type="text0" value="<?=$collss['producto']." (".$collss['cantidad'].")"?>" class="form-control" name="" readonly>
                      </div>
                      <div class="col-xs-12 col-sm-6 col-md-4">
                        <input type="number" value="<?=$collss['cantidad_productos']?>" class="form-control cant<?php echo $collss['id_producto'] ?>" name="" readonly="">
                      </div>
                      <div class="col-xs-12 col-sm-6 col-md-4">
                        <input type="hidden" value="<?=$collss['id_producto']?>" name="id_producto[]">
                        <input type="number" <?php foreach ($desperfectos as $keys): if(!empty($keys['id_producto'])): 
                        if ($keys['id_producto']==$collss['id_producto']): ?> value="<?=$keys['cantidad_desperfectos']?>" <?php else:?>  <?php endif; endif; endforeach; ?> value="0" min="0" max="<?=$collss['cantidad_productos']*$pedido['cantidad_aprobado'] ?>" id="<?php echo $collss['id_producto'] ?>" class="form-control cantidades" name="cantidad_desperfectos[]">
