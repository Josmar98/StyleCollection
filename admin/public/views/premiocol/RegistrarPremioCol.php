        ...
        <div class="col-xs-12" >
          ...
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">Agregar <?php echo "Premios de colecciones"; ?></h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
                ...
                ?>
                  <div class="row">
                    <div class="col-sm-4">
                      <div class="row">
                          <div class="form-group col-sm-12">
                             <label for="plan">Plan <?=$plan['nombre_plan']?></label>
                             <input type="text" class="form-control" id="plan<?=$plan['id_plan']?>" name="plan[]" value="<?=$plan['nombre_plan'];?>" readonly style='font-size:1.3em'>
                             <span id="error_plan<?=$plan['id_plan']?>" class="errors"></span>
                          </div>
                          <div class="form-group col-sm-12">
                             <label for="cantidad_plan<?=$plan['id_plan']?>">Cantidad de planes</label>
                             <input type="number" step="1" min="0" class="form-control aprobado<?=$plan['id_plan']?>" id="<?=$plan['id_plan'];?>" name="cantidad_plan[]" readonly value="<?=$plan['cantidad_coleccion_plan']?>">
                    ...
                    <div class="form-group col-sm-8">
                      <?php
                        foreach ($tipo_premios_planes as $key1) {
                          if(!empty($key1['id_plan_campana'])){
                              ...
                              <div class="row">
                                ...
                                <div class="form-group col-sm-6">
                                  <label for="">Premio</label>
                                   <input type="text" class="form-control " id="" name="premios[]" value="<?=$key1['nombre_premio']?>" readonly>


                                    <span class="existenciaDisponible<?=$plan['id_plan'].$key1['id_premio']?> errors"></span>

                                   <?php foreach ($existencias as $exiss) {
                                     if(!empty($exiss['id_premio'])){
                                        if($exiss['id_premio'] == $key1['id_premio']){
                                          ?>
                                          <input type="hidden" name="id_existencia[]" value="<?=$exiss['id_existencia']?>">
                                  <input type="hidden" id="existencia<?=$plan['id_plan'].$key1['id_premio']?>" name="existenciaAct[]" value="<?=$exiss['cantidad_existencia']?>">
                                  <input type="hidden" id="newexistencia<?=$plan['id_plan'].$key1['id_premio']?>" name="existenciaNew[]" value="<?=$exiss['cantidad_existencia']?>">
                                          <?php
                                        }
                                     }
                                   } ?>
                                  ...
                                  <input type="hidden" name="id_tppc[]" value="<?=$key1['id_tppc']?>">

                                </div>

                                <div class="form-group col-sm-6">
                                   <label for="<?=$plan['id_plan'].$key1['id_premio']?>">Cantidad de premios</label>
                                   <input type="number" step="1" min="0" class="form-control cantidades cant<?=$plan['id_plan'].$key1['id_premio']?>" id="<?=$plan['id_plan'].$key1['id_premio'];?>" name="cantidad_premios_plan[]" <?php if($plan['cantidad_coleccion_plan']==0){echo "readonly";}?> value="0">
                                   <input type="hidden" id="llenar<?=$plan['id_plan'].$key1['id_premio'];?>" value="0">
                            ...
                            <?php 
                            $planesss[$j] = $plan['id_plan'].$key1['id_premio'];
                            // $planesss3[$j] = $key1['id_premio'];
                            $j++; 
                            // print_r($tipo_premios_planespp[$conta]);
                            // echo "<br><br>";
                            // $conta++;
                            }
                          }
                  ...
                  <?php
                    ...
                    <?php $planesss3 = []; $x = 0; ?>
                    <?php if(!empty($tipo_premios_planespp)){ ?>
                    <?php foreach ($tipo_premios_planespp as $keys3): ?>
                      <?php if (!empty($keys3['id_premio'])): ?>
                            <?php $planesss3[$x] = $keys3['id_premio']; ?>
                        <?php $x++; ?>
                      <?php endif ?>
                    <?php endforeach ?>
                    <?php } ?>
