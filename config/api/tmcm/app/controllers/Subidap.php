<?php
	if(is_file('app/models/tsubidap.php')){ require_once'app/models/tsubidap.php'; }
	...
	if(!empty($action)){
						...
						if(count($search['data'])>0){
							...
							if( ($general['CantTotal']>=$searchCantG) && ($general['CantM']>=$searchCantM) && ($general['CantF']>=$searchCantF) ){
								...
								if($resultExec['msj']=="Good"){
									...
									$resultExec3 = $app->EliminarPastores($data);
									if($resultExec3['msj']=="Good"){
										$data['id_users'] = $idUser;
										$data['id_dominio'] = $idDomain;
										$errores = 0;
										$pastores = $_POST['pastores'];
										foreach ($pastores as $keys) {
											$data['nombre_pastor'] = $keys['nombre'];
											$data['cantIgM'] = $keys['CantM'];
											$data['cantIgF'] = $keys['CantF'];
											$data['cantIgG'] = $keys['CantTotal'];
											$resultExec2 = $app->RegistrarPastor($data);
				...
				if($results['msj']=="Good"){
					if(count($results['data'])>0){
						...
						if($totalGeneral['cantM']!=null && $totalGeneral['cantF']!=null && $totalGeneral['cantG']!=null){
							...
							$pastores = $app->ConsultarListaPastores();
							$index = 0;
							$return['Pastores'] = [];
							foreach ($pastores['data'] as $iglesia) {
								$iglesia = $iglesia['nombre_pastor'];
								$resultsIg = $app->ConsultarBajadasPastores($dominio, $iglesia);
								if($resultsIg['msj']=="Good"){
									if(count($resultsIg['data'])>0){
										$return['msjPastores'] = "Good";
										$totalIg = $resultsIg['data'][$index];
										$return['Pastores'][$iglesia]['cantM']=$totalIg['cantM'];
										$return['Pastores'][$iglesia]['cantF']=$totalIg['cantF'];
										$return['Pastores'][$iglesia]['cantG']=$totalIg['cantG'];
									}else{
										$return['msjPastores'] = "Vacio";
									}
								}
							}
						}else{
		...
		// 					$return['General']['cantF']=$totalGeneral['cantF'];
		// 					$return['General']['cantG']=$totalGeneral['cantG'];
		// 					$pastores = $app->ConsultarListaIglesias();
		// 					$index = 0;
		// 					$return['Pastores'] = [];
		// 					foreach ($pastores['data'] as $iglesia) {
		// 						$iglesia = $iglesia['nombre_pastor'];
