...
if($_SESSION['nombre_rol']=="Superusuario" || $_SESSION['nombre_rol']=="Administrador"){	
	...
	if(!empty($_GET['permission']) && $_GET['permission'] == 1 ){
		$query = "UPDATE obsequiogemas SET estatus = 1 WHERE id_obsequio_gema = $id";
		...
		if($res1['ejecucion']==true){
			...
			if(!empty($modulo) && !empty($accion)){
				$fecha = date('Y-m-d');
				$hora = date('H:i:a');
				$query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora) VALUES (DEFAULT, {$_SESSION['id_usuario']}, 'Obsequio Gemas', 'Restaurar', '{$fecha}', '{$hora}')";
	...
	if(empty($_POST)){
		$obsequios = $lider->consultarQuery("SELECT * FROM clientes, obsequiogemas, campanas, despachos WHERE clientes.id_cliente = obsequiogemas.id_cliente and campanas.id_campana = obsequiogemas.id_campana and despachos.id_campana = campanas.id_campana and despachos.id_despacho = obsequiogemas.id_despacho and despachos.id_despacho = {$id_despacho} and campanas.id_campana = {$id_campana} and clientes.estatus = 1 and obsequiogemas.estatus = 0");
		// print_r($obsequios);
		if(!empty($action)){
