        ...
        $response = "1";
          ...
          if(!empty($modulo) && !empty($accion)){
            ...
            $elementos = array(
              ...
              "Actual"=> [ 0=> $id, 1=> $nombre1, 2=> $nombre2 , 3=>$apellido1, 4=>$apellido2, 5=>$campAnt['cod_cedula'], 6=>$cedula, 7=>$sexo, 8=>$lider->formatFecha($fechaNacimiento), 9=>$telefono, 10=>$telefono2, 11=>$correo, 12=>$cod_rif, 13=>$rif, 14=>$direccion, 15=>$id_lider, 16=>"1"]
            );
            $elementosJson = json_encode($elementos, JSON_UNESCAPED_UNICODE, JSON_UNESCAPED_SLASHES);

            $fecha = date('Y-m-d');
            $hora = date('H:i:a');
            $query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora, elementos) VALUES (DEFAULT, {$_SESSION['id_usuario']}, 'Clientes', 'Editar', '{$fecha}', '{$hora}', '{$elementosJson}')";
