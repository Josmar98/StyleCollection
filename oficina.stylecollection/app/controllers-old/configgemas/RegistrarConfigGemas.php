...
if($_SESSION['nombre_rol']=="Superusuario" || $_SESSION['nombre_rol']=="Administrador"){
      ...
      if(!empty($_POST['nombre']) && !empty($_POST['cantidadGemas']) && !empty($_POST['condicion']) ){
        ...
        if(count($buscar)>1){
          ...
          $query = "INSERT INTO configgemas (id_configgema, nombreconfiggema, cantidad_correspondiente, condicion, estatus) VALUES (DEFAULT, '{$nombre}', '{$cantidad}', '{$condicion}', 1)";
          $exec = $lider->registrar($query, "configgemas", "id_configgema");
          if($exec['ejecucion']==true){
            $response = "1";
                    if(!empty($modulo) && !empty($accion)){
                      $fecha = date('Y-m-d');
                      $hora = date('H:i:a');
                      $query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora) VALUES (DEFAULT, {$_SESSION['id_usuario']}, 'Configuracion de Gemas', 'Registrar', '{$fecha}', '{$hora}')";
