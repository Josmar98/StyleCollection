...
$id_nota=$_GET['id'];
$query = "SELECT * FROM clientes, factura_personalizada WHERE factura_personalizada.id_cliente = clientes.id_cliente and factura_personalizada.id_factura_personalizada={$id_nota}";
$notaP = $lider->consultarQuery($query);
$notaP = $notaP[0];
			...
			$factura = $lider->consultarQuery($query);
			if(count($factura)>1){
			...
			$buscarFacturasVariadas = $lider->consultarQuery("SELECT * FROM lista_factura_personalizada WHERE id_factura_personalizada = {$id_nota}");
				...
				if(!empty($variadas['id_factura_personalizada'])){
					...
					foreach ($buscarRubro as $keys) {
						if(!empty($keys['estatus'])){
							$longCol = count($opcionesEntregas);
							$opcionesEntregas[$longCol] = $variadas;
							// $opcionesEntregas[$longCol]['cantidad_productos'] = $opcionesEntregas[$longCol]['cantidades'];
							$opcionesEntregas[$longCol]['precio_producto'] = $opcionesEntregas[$longCol]['precios'];
							foreach ($keys as $key => $value) {
								if(is_string($key)){
									$opcionesEntregas[$longCol][$key] = $value;
								}
							}
							if($opcionesEntregas[$longCol]['tipos']=="Productos"){
								$opcionesEntregas[$longCol]['rubro'] = $opcionesEntregas[$longCol]['producto'];
							}
							if($opcionesEntregas[$longCol]['tipos']=="Premios"){
								$opcionesEntregas[$longCol]['rubro'] = $opcionesEntregas[$longCol]['nombre_premio'];
							}
			...
			// 		$opcionesEntregas[count($opcionesEntregas)]=$key;
			// 	}
			// }

			$opcionesEntregas[count($opcionesEntregas)]=['estatus'=>true];
					...
					<div class='box-body'>
	                    <div class='text-center' style='width:60%;text-center;display:inline-block;'>
	                      ...
	                      <div style='border:none;min-width:100%;max-width:100%;min-height:50px;max-height:50px;text-align:center;padding:0'>
	                      	AV LOS HORCONES ENTRE CALLES 9 Y 10 LOCAL NRO S/N BARRIO PUEBLO
	                      	NUEVO BARQUISIMETO EDO LARA ZONA POSTAL 3001 
	                      </div>
	                    </div>
	                    ...
	                    <br>
	                      ...
	                      <table class='table table-bordered text-left' >
	                        ...
	                        <tbody>";
	                        	foreach($opcionesEntregas as $entregas){ if(!empty($entregas['id_factura_personalizada'])){
		                            ...
		                            $info .= "<tr>
		                              ...
		                              <td class='col2'>";
		                                ...
		                                $id_p = "";
		                                $name_p = "";
		                                if($entregas['tipos']=="Productos"){
		                                  // $premio_producto = $productos;
		                                  $id_p = "id_producto";
		                                  $name_p = "producto";
		                                }
		                                if($entregas['tipos']=="Premios"){
		                                ...
		                                //     if($premio[$id_p]==$entregas[$name_p]){
		                                $info .= $entregas[$name_p];
		                                //     }
		                                //   }
		                                // }
