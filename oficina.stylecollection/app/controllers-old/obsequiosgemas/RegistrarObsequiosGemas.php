...
if($estado_campana=="1"){
		...
		if(!empty($_POST['clientes']) && !empty($_POST['cantidad_gemas'])){
			...
			// 	$response = "9";
			// }else{
				$query = "INSERT INTO obsequiogemas (id_obsequio_gema, id_cliente, id_pedido, id_campana, id_despacho, cantidad_gemas, descripcion_gemas, fecha_obsequio, hora_obsequio, firma_obsequio, estatus) VALUES (DEFAULT, {$id_cliente}, {$id_pedido}, {$id_campana}, {$id_despacho}, '{$gemas}', '{$descripcion}', '{$fecha}', '{$hora}', '{$name}', 1)";
				$exec = $lider->registrar($query, "obsequiogemas", "id_obsequio_gema");
				if($exec['ejecucion']==true){
					$response = "1";
					if(!empty($modulo) && !empty($accion)){
						$fecha = date('Y-m-d');
						$hora = date('H:i:a');
						$query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora) VALUES (DEFAULT, {$_SESSION['id_usuario']}, 'Obsequio Gemas', 'Registrar', '{$fecha}', '{$hora}')";
