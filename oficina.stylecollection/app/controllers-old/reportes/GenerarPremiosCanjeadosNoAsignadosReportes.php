...
if($amReportesC == 1){
  if(isset($_GET['C']) || isset($_GET['ID'])){
    ...
    if(isset($_GET['C'])){
      if($_GET['C'] == 0){
        $listado = $lider->consultarQuery("SELECT * FROM canjeos, catalogos WHERE catalogos.id_catalogo = canjeos.id_catalogo and canjeos.estatus = 1 and canjeos.estado_canjeo IS NULL");
      }
      if($_GET['C'] > 0){
        $listado = $lider->consultarQuery("SELECT * FROM canjeos, catalogos WHERE catalogos.id_catalogo = canjeos.id_catalogo and canjeos.estatus = 1 and canjeos.estado_canjeo IS NULL and catalogos.cantidad_gemas = {$_GET['C']}");
      }
    }
    if(isset($_GET['ID'])){
      if($_GET['ID'] == 0){
      ...
      if($_GET['ID'] > 0){
        $listado = $lider->consultarQuery("SELECT * FROM canjeos, catalogos WHERE catalogos.id_catalogo = canjeos.id_catalogo and canjeos.estatus = 1 and canjeos.estado_canjeo IS NULL and catalogos.id_catalogo = {$_GET['ID']}");
      }
