...
if($estado_campana=="1"){
	...
	if(!empty($_POST['cantidad_plan']) && !empty($_POST['id_pedido'])){
		...
		$i=0;
		$responses = [];
		$exec = $lider->eliminar("DELETE FROM retos WHERE id_cliente = {$id_cliente} and id_campana = {$id_campana}");
		// $exec = $lider->eliminar("DELETE FROM retos WHERE id_pedido = {$id_pedido} and id_cliente = {$id_cliente} and id_campana = {$id_campana}");
		...
		$pedido = $lider->consultarQuery("SELECT * FROM despachos, pedidos, clientes WHERE despachos.id_despacho = pedidos.id_despacho and pedidos.id_cliente = clientes.id_cliente and despachos.id_campana = $id_campana and pedidos.id_pedido = {$id}");
		// $pedido = $lider->consultarQuery("SELECT * FROM pedidos, clientes WHERE pedidos.id_cliente = clientes.id_cliente and pedidos.id_despacho = $id_despacho and pedidos.id_pedido = {$id}");
		$pedido = $pedido[0];
		$pedido2 = $lider->consultarQuery("SELECT * FROM despachos, pedidos, clientes WHERE despachos.id_despacho = pedidos.id_despacho and pedidos.id_cliente = clientes.id_cliente and despachos.id_campana = {$id_campana} and clientes.id_cliente = {$pedido['id_cliente']}");
		// $pedido = $lider->consultarQuery("SELECT * FROM pedidos, clientes WHERE pedidos.id_cliente = clientes.id_cliente and pedidos.id_despacho = $id_despacho and pedidos.id_pedido = {$id}");
		$pedido = $lider->consultarQuery("SELECT * FROM despachos, pedidos, clientes WHERE despachos.id_despacho = pedidos.id_despacho and pedidos.id_cliente = clientes.id_cliente and despachos.id_campana = $id_campana and pedidos.id_pedido = {$id}");
		...
		if(Count($pedido)>1){
			...
			$retosLider = $lider->consultarQuery("SELECT * FROM retos, retos_campana, premios WHERE retos.id_reto_campana = retos_campana.id_reto_campana and retos.id_pedido = {$id} and retos_campana.id_premio = premios.id_premio and retos_campana.estatus = 1 and retos_campana.id_campana = {$id_campana}");
			foreach ($pedido2 as $keys) {
			...
			// ======================================= // ======================================== // =================================== //
				...
				// ============================== // ================================== // ============================== //
			// ======================================= // ======================================== // =================================== //
			// $disponible = $pedido['cantidad_aprobado']; 
			$disponible = $cantidad_colecciones; 
			$disponible = $coleccionesPuntualesRed; 
			foreach ($retosLider as $key) {
				$res = $key['cantidad_retos']*$key['cantidad_coleccion'];
				$disponible -= $res;
			}
	...
	if(empty($_POST)){
		...
		if(Count($pedido)>1){
			...
			// $disponible = $pedido['cantidad_aprobado']; 
			$cantidad_colecciones = $coleccionesPuntualesRed;
			$disponible = $cantidad_colecciones;
			$disponible = $coleccionesPuntualesRed;
			foreach ($retosLider as $key) {
				$res = $key['cantidad_retos']*$key['cantidad_coleccion'];
				// echo "Cant: ".$key['cantidad_coleccion']."<br>";
				// echo "Cant: ".$key['cantidad_retos']."<br>";
				// echo "<br>";
				$disponible -= $res;
			}
		}
