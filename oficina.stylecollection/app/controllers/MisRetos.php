...
if(empty($_POST)){
	...
	$lideres = $lider->consultarQuery("SELECT DISTINCT clientes.id_cliente, primer_nombre, primer_apellido, cedula, pedidos.id_pedido FROM clientes, retos, pedidos WHERE pedidos.id_pedido = retos.id_pedido and clientes.id_cliente = retos.id_cliente and retos.id_campana = $id_campana and clientes.id_cliente = $id");

	$retos = $lider->consultarQuery("SELECT * FROM retos, retos_campana, premios WHERE retos.id_reto_campana = retos_campana.id_reto_campana and retos_campana.id_premio = premios.id_premio and retos_campana.id_campana = $id_campana and retos_campana.estatus = 1");
	if(Count($lideres)>1){
