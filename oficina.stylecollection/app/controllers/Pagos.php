...
if($_SESSION['nombre_rol']!="Vendedor"){	
	...
	if(!empty($_POST['cantidadDistribucion']) && !empty($_POST['concepto_pago']) && !empty($_POST['id_pedido'])){
		$id_pedidoDist = $_POST['id_pedido'];
		$concepto_pagoDist = $_POST['concepto_pago'];
		$cantidadDist = 0;
		if($_POST['cantidad']!=""){
			$cantidadDist = $_POST['cantidad'];
		}
		$busqueda = $lider->consultarQuery("SELECT * FROM distribucion_pagos WHERE id_pedido = {$id_pedidoDist} and tipo_distribucion = '{$concepto_pagoDist}'");
		if(count($busqueda)<2){
			$sql = "INSERT INTO distribucion_pagos (id_distribucion_pagos, id_pedido, tipo_distribucion, cantidad_distribucion, estatus) VALUES (DEFAULT, {$id_pedidoDist}, '{$concepto_pagoDist}', '{$cantidadDist}', 1)";
			$exec = $lider->registrar($sql, "distribucion_pagos", "id_distribucion_pagos");
			...
			$idDist = $busqueda[0]['id_distribucion_pagos'];
			$sql = "UPDATE distribucion_pagos SET cantidad_distribucion='{$cantidadDist}', estatus=1 WHERE id_pedido = {$id_pedidoDist} and tipo_distribucion='{$concepto_pagoDist}' and id_distribucion_pagos={$idDist}";
			$exec = $lider->modificar($sql);
			if($exec['ejecucion']==true){
				$response = "1";
			}else{
				$response = "2";
		...
		if($tasa!=""){
			if($pago['forma_pago']=="Divisas Dolares" || $pago['forma_pago']=="Divisas Euros" || $pago['forma_pago']=="Deposito En Dolares"){
				if($pago['estado']=="Abonado"){
					$exec = $lider->modificar("UPDATE pagos SET id_pedido={$id_pedido_temp}, fecha_pago = '{$fecha}', tasa_pago='', tipo_pago='{$tipo_pago}' WHERE id_pago='$id_pago'");
				}else{
					if($fecha == $pago['fecha_pago']){
						$exec = $lider->modificar("UPDATE pagos SET id_pedido={$id_pedido_temp}, fecha_pago = '{$fecha}', tasa_pago=null, tipo_pago='{$tipo_pago}' WHERE id_pago='$id_pago'");
					}else{
						$exec = $lider->modificar("UPDATE pagos SET id_pedido={$id_pedido_temp}, fecha_pago = '{$fecha}', tasa_pago=null, tipo_pago='{$tipo_pago}', estado = '' WHERE id_pago='$id_pago'");
					}
				}
			}else{
				if(count($tasass)>1){
				...
				if($pago['estado']=="Abonado"){
					...
					if($fecha == $pago['fecha_pago']){
						...
						$exec = $lider->modificar("UPDATE pagos SET id_pedido={$id_pedido_temp}, fecha_pago = '{$fecha}', tasa_pago='{$tasa}', tipo_pago='{$tipo_pago}', equivalente_pago='{$equivalente_pago}', estado = '' WHERE id_pago='$id_pago'");
					}
				}
			}
		}else{
			if($pago['estado']=="Abonado"){
				$exec = $lider->modificar("UPDATE pagos SET id_pedido={$id_pedido_temp}, fecha_pago = '{$fecha}', tipo_pago='{$tipo_pago}' WHERE id_pago='$id_pago'");
			}else{
				if($fecha == $pago['fecha_pago']){
					$exec = $lider->modificar("UPDATE pagos SET id_pedido={$id_pedido_temp}, fecha_pago = '{$fecha}', tipo_pago='{$tipo_pago}' WHERE id_pago='$id_pago'");
				}else{
					$exec = $lider->modificar("UPDATE pagos SET id_pedido={$id_pedido_temp}, fecha_pago = '{$fecha}', tipo_pago='{$tipo_pago}', estado = '' WHERE id_pago='$id_pago'");
				}
			}
		}
	...
	if(empty($_POST)){
		...
		if(Count($pedidos)>1){
			...
			if(count($liderazgos)>1){
				...
				foreach ($liderazgosAll as $data){ if(!empty($data['id_liderazgo'])){
						// echo $lidera['nombre_liderazgo']." ==== ";
						// echo $data['nombre_liderazgo']."<br>";
					if ($lidera['id_liderazgo'] >= $data['id_liderazgo']){
						$resultNLider = $data['descuento_coleccion']*$pedido['cantidad_aprobado'];
						// echo "<br> Descuento Coleccion: ".$data['descuento_coleccion'];
			...
			$pagosRTomarEnCuenta = "";
			$numberIndex1 = 0;
			foreach ($pagosRecorridos as $key) {
				$pagosRTomarEnCuenta .= "'".$key['name']."'";
				if($numberIndex1<=(count($pagosRecorridos)-2) ){
					$pagosRTomarEnCuenta .= ", ";
				}
				$numberIndex1++;
			}
			// echo $pagosRTomarEnCuenta."<br>";
			// print_r($pagosRecorridos);
			$distribucionPagos = $lider->consultarQuery("SELECT * FROM distribucion_pagos WHERE id_pedido = {$pedido['id_pedido']} and tipo_distribucion in ($pagosRTomarEnCuenta)");


			$promocionesEx = $lider->consultarQuery("SELECT * FROM promocion, promociones WHERE promocion.id_promocion = promociones.id_promocion and promociones.id_cliente = {$id_cliente} and promocion.id_campana = {$id_campana} and promociones.id_despacho = {$id_despacho}");

			$PromosTomarEnCuenta = "";
			$numberIndex2 = 0;
			foreach ($promocionesEx as $promoPagos){ if(!empty($promoPagos['id_promocion'])){
				$PromosTomarEnCuenta .= "'".$promoPagos['nombre_promocion']."'";
				if($numberIndex2<=(count($promocionesEx)-3) ){
					$PromosTomarEnCuenta .= ", ";
				}
				$numberIndex2++;
			} }
		...
		// echo "<br>".$resulttDescuentoNivelLider."<br>";
		// echo "<br>".$resulttDescuentoDirecto."<br>";
		// echo "<br>".$bonoContado1Puntual."<br>";
		// echo "<br>".$bonoPagosPuntuales."<br>";
		// echo "<br>".$totalTraspasoRecibido."<br>";
		// echo "<br>".$bonoAcumuladoCierreEstructura."<br>";
		// echo "<br>".$liquidacion_gemas."<br>";
		...
		if($pagos['ejecucion']==1){
			...
			if(!empty($_GET['lider'])){
				...
				foreach ($distribucionPromos as $key) { 
					if(!empty($key['id_distribucion_pagos'])){
				...
				foreach ($promociones as $promoPagos) {
					if(!empty($promoPagos['id_promocion'])){
						if ($promoPagos['id_promocion'] == $_GET['distribucionConceptoEx']){
							$conceptoDist = $promoPagos['nombre_promocion'];
						}
					}
				}
				$busquedaDist = $lider->consultarQuery("SELECT * FROM distribucion_pagos WHERE id_pedido = {$pedido['id_pedido']} and tipo_distribucion = '{$conceptoDist}'");
