...
if($_SESSION['nombre_rol']!="Vendedor"){	
	...
	if(!empty($_POST['id_pago_modal']) && !empty($_POST['fecha_pago']) && !empty($_POST['rol']) && $_POST['rol']=="Analistas" && !empty($_POST['tipo_pago']) && isset($_POST['tasa'])){
		...
		if(count($paggo)>1){
			...
			$return['pago']['tasa_pago_format'] = number_format($paggo[0]['tasa_pago'],2,',','.');
		...
		if(!empty($_GET['admin'])&&!empty($_GET['lider'])){
			...
			if(!empty($_GET['rangoI']) && !empty($_GET['rangoF'])){
				...
				if(!empty($_GET['Banco'])){
					...
					$id_banco = $_GET['Banco'];
					$pagos = $lider->consultarQuery("SELECT * FROM campanas, despachos, pedidos, pagos WHERE campanas.id_campana = despachos.id_despacho and campanas.estatus = 1 and despachos.estatus = 1 and despachos.id_despacho = pedidos.id_despacho and pedidos.estatus = 1 and pedidos.id_pedido = pagos.id_pedido and pagos.estatus = 1 and pedidos.id_cliente = {$id_cliente} and campanas.id_campana = {$id_campana} and despachos.id_despacho = {$id_despacho} and pagos.id_banco = {$id_banco} and (pagos.forma_pago = 'Divisas Dolares' or pagos.forma_pago = 'Divisas Euros') and pagos.fecha_pago BETWEEN '$rangoI' and '$rangoF' ORDER BY fecha_pago asc");
				}else{
					$pagos = $lider->consultarQuery("SELECT * FROM campanas, despachos, pedidos, pagos WHERE campanas.id_campana = despachos.id_despacho and campanas.estatus = 1 and despachos.estatus = 1 and despachos.id_despacho = pedidos.id_despacho and pedidos.estatus = 1 and pedidos.id_pedido = pagos.id_pedido and pagos.estatus = 1 and pedidos.id_cliente = {$id_cliente} and campanas.id_campana = {$id_campana} and despachos.id_despacho = {$id_despacho} and (pagos.forma_pago = 'Divisas Dolares' or pagos.forma_pago = 'Divisas Euros') and pagos.fecha_pago BETWEEN '$rangoI' and '$rangoF' ORDER BY fecha_pago asc");
				}
			}else{
				if(!empty($_GET['Banco'])){
					$id_banco = $_GET['Banco'];
					$pagos = $lider->consultarQuery("SELECT * FROM campanas, despachos, pedidos, pagos WHERE campanas.id_campana = despachos.id_despacho and campanas.estatus = 1 and despachos.estatus = 1 and despachos.id_despacho = pedidos.id_despacho and pedidos.estatus = 1 and pedidos.id_pedido = pagos.id_pedido and pagos.estatus = 1 and pedidos.id_cliente = {$id_cliente} and campanas.id_campana = {$id_campana} and despachos.id_despacho = {$id_despacho} and pagos.id_banco = {$id_banco} and (pagos.forma_pago = 'Divisas Dolares' or pagos.forma_pago = 'Divisas Euros') ORDER BY fecha_pago asc");

				}else{
					$pagos = $lider->consultarQuery("SELECT * FROM campanas, despachos, pedidos, pagos WHERE campanas.id_campana = despachos.id_despacho and campanas.estatus = 1 and despachos.estatus = 1 and despachos.id_despacho = pedidos.id_despacho and pedidos.estatus = 1 and pedidos.id_pedido = pagos.id_pedido and pagos.estatus = 1 and pedidos.id_cliente = {$id_cliente} and campanas.id_campana = {$id_campana} and despachos.id_despacho = {$id_despacho} and (pagos.forma_pago = 'Divisas Dolares' or pagos.forma_pago = 'Divisas Euros') ORDER BY fecha_pago asc");
				}
			}
		}else{
			if(!empty($_GET['rangoI']) && !empty($_GET['rangoF'])){
				...
				if(!empty($_GET['Banco'])){
					...
					$pagos = $lider->consultarQuery("SELECT * FROM campanas, despachos, pedidos, pagos WHERE campanas.id_campana = despachos.id_despacho and campanas.estatus = 1 and despachos.estatus = 1 and despachos.id_despacho = pedidos.id_despacho and pedidos.estatus = 1 and pedidos.id_pedido = pagos.id_pedido and pagos.estatus = 1 and campanas.id_campana = {$id_campana} and despachos.id_despacho = {$id_despacho} and (pagos.forma_pago = 'Divisas Dolares' or pagos.forma_pago = 'Divisas Euros') and pagos.fecha_pago BETWEEN '$rangoI' and '$rangoF' ORDER BY fecha_pago asc");
				}
			}else{
				if(!empty($_GET['Banco'])){
					$id_banco = $_GET['Banco'];
					...
					$pagos = $lider->consultarQuery("SELECT * FROM campanas, despachos, pedidos, pagos WHERE campanas.id_campana = despachos.id_despacho and campanas.estatus = 1 and despachos.estatus = 1 and despachos.id_despacho = pedidos.id_despacho and pedidos.estatus = 1 and pedidos.id_pedido = pagos.id_pedido and pagos.estatus = 1 and campanas.id_campana = {$id_campana} and despachos.id_despacho = {$id_despacho} and (pagos.forma_pago = 'Divisas Dolares' or pagos.forma_pago = 'Divisas Euros') ORDER BY fecha_pago asc");
				}
			}
