...
if($_SESSION['nombre_rol']=="Superusuario" || $_SESSION['nombre_rol']=="Administrador" || $_SESSION['nombre_rol']=="Administrativo" || $_SESSION['nombre_rol']=="Contable"){
	...
	$proveedores=$lider->consultar("proveedores_compras");
	...
	if(!empty($_GET['permission']) && $_GET['permission'] == 1 ){
			...
			$query = "UPDATE proveedores_compras SET estatus = 0 WHERE id_proveedor_compras = {$id}";
			...
			if($res1['ejecucion']==true){
				...
				// }
		            ...
		            if(!empty($modulo) && !empty($accion)){
		              $fecha = date('Y-m-d');
		              $hora = date('H:i:a');
		              $query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora) VALUES (DEFAULT, {$_SESSION['id_usuario']}, 'Proveedores', 'Borrar', '{$fecha}', '{$hora}')";
