...
if(!empty($_POST['id_producto']) && !empty($_POST['cantidad_desperfectos'])){
	...
	$buscar = $lider->consultarQuery("DELETE FROM notificar_desperfectos WHERE id_pedido = {$id_pedido}");
	if($buscar['ejecucion'] == 1){
		...
		foreach ($cantidad_desperfectos as $cantDes) {
			if($cantDes>0){
				...
				// echo "<br><br>";
				// echo "Pedido: ".$id_pedido." ||| Desperfecto: ".$id_desperfecto.' ||| Producto: '.$id_producto[$i]." ||| Cantidad: ".$cantDes;
				$query = "INSERT INTO notificar_desperfectos (id_notificar_desperfecto, id_desperfecto, id_pedido, id_producto, cantidad_desperfectos, estatus) VALUES (DEFAULT, {$id_desperfecto}, {$id_pedido}, {$id_producto[$i]}, {$cantDes}, 1)";
		...
		if($response=="1"){
			if(!empty($modulo) && !empty($accion)){
				$fecha = date('Y-m-d');
				$hora = date('H:i:a');
				$query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora) VALUES (DEFAULT, {$_SESSION['id_usuario']}, 'Notificar Desperfectos', 'Editar', '{$fecha}', '{$hora}')";
	...
	$desperfecto = $lider->consultarQuery("SELECT * FROM desperfectos WHERE id_campana = {$id_campana}");
		...
		$pedido = $pedido[0];
	$desperfectos = $lider->consultarQuery("SELECT * FROM notificar_desperfectos, productos, desperfectos WHERE notificar_desperfectos.id_producto = productos.id_producto and notificar_desperfectos.id_desperfecto = desperfectos.id_desperfecto and notificar_desperfectos.id_pedido = {$pedido['id_pedido']}");
	if(!empty($action)){
