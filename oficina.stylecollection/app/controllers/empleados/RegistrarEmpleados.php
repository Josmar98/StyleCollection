...
if($amEmpleadosR == 1){
    ...
    if(!empty($_POST['validarData'])){
      ...
      if($res1['ejecucion']==true){
        if(Count($res1)>1){
          ...
          $res2 = $lider->consultarQuery("SELECT * FROM empleados WHERE cedula = '$cedula' and estatus = 0");
          if($res2['ejecucion']==true){
            if(Count($res2)>1){
              $res3 = $lider->modificar("UPDATE empleados SET estatus = 1 WHERE cedula = '$cedula'");
    ...
    if(!empty($_POST['nombre1']) && !empty($_POST['cedula'])){
      ...
      $query = "INSERT INTO empleados (id_empleado, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, cod_cedula, cedula, sexo, fecha_nacimiento, telefono, telefono2, correo, cod_rif, rif, direccion, estatus) VALUES (DEFAULT, '$nombre1', '$nombre2', '$apellido1', '$apellido2', '$cod_cedula', '$cedula', '$sexo', '$fechaNacimiento', '$telefono', '$telefono2', '$correo', '$cod_rif', '$rif', '$direccion', 1)";
      // echo $query;
      // die();
      $exec = $lider->registrar($query, "empleados", "id_empleado");
        ...
        $response = "1";
          if(!empty($modulo) && !empty($accion)){
            ...
            $elementos = array(
              ...
              "Anterior"=> "",
              "Actual"=> [ 0=> $id, 1=> $nombre1, 2=> $nombre2 , 3=>$apellido1, 4=>$apellido2, 5=>$cod_cedula, 6=>$cedula, 7=>$sexo, 8=>$lider->formatFecha($fechaNacimiento), 9=>$telefono, 10=>$telefono2, 11=>$correo, 12=>$cod_rif, 13=>$rif, 14=>$direccion, 15=>"1"]
            );
            $elementosJson = json_encode($elementos, JSON_UNESCAPED_UNICODE, JSON_UNESCAPED_SLASHES);

            $fecha = date('Y-m-d');
            $hora = date('H:i:a');
            $query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora, elementos) VALUES (DEFAULT, {$_SESSION['id_usuario']}, 'Empleados', 'Registrar', '{$fecha}', '{$hora}', '{$elementosJson}')";
