...
if($_SESSION['nombre_rol']=="Administrador" || $_SESSION['nombre_rol']=="Superusuario" || $_SESSION['nombre_rol']=="Analista" || $_SESSION['nombre_rol']=="Analista Supervisor" || $_SESSION['nombre_rol']=="Administrativo"){
	...
	if($estado_campana=="1"){
		...
		if(!empty($_POST['pedido'])){
			...
			$maxNumero = $lider->consultarQuery("SELECT MAX(factura_despacho_personalizada.numero_factura) as numero_factura FROM notasentregapersonalizada WHERE estatus=1");
			...
			// $buscar = $lider->consultarQuery("SELECT * FROM factura_despacho_personalizada WHERE id_pedido = {$id_pedido} AND estatus = 1");
			// if(count($buscar)>1){
			// 	$response = "9";
			// }else{
			// 	$fechaHoraActual = date('Y-m-d H:i:s');
			// 	$query = "INSERT INTO factura_despacho_personalizada (id_factura_despacho_perso, id_pedido, id_almacen, numero_factura, tipo_factura, fecha_emision, fecha_vencimiento, numero_control1, numero_control2, fecha_creacion, estado_factura, estatus) VALUES (DEFAULT, {$id_pedido}, {$id_almacen}, {$numero_factura}, '{$forma_pago}', '{$fecha_emision}', '{$fecha_vencimiento}', {$control1}, {$control2}, '{$fechaHoraActual}', 1, 1)";
			// 	// echo $query;
			// 	$exec = $lider->registrar($query, "factura_despacho_personalizada", "id_factura_despacho_perso");
			...
			// 			if($proceder){
			// 				foreach ($pedidosVariados as $pedidoVariado) {
			// 					$query = "INSERT INTO factura_despacho_variadas_perso (id_factura_despacho_variada_perso, id_factura_despacho_perso, id_pedido_factura, estatus) VALUES (DEFAULT, {$id_factura_despacho}, {$pedidoVariado}, 1);";
			...
			// }
			$fechaHoraActual = date('Y-m-d H:i:s');
			$query = "INSERT INTO factura_despacho_personalizada (id_factura_despacho_perso, id_pedido, id_almacen, numero_factura, tipo_factura, fecha_emision, fecha_vencimiento, numero_control1, numero_control2, fecha_creacion, estado_factura, observacion, estatus) VALUES (DEFAULT, {$id_pedido}, {$id_almacen}, {$numero_factura}, '{$forma_pago}', '{$fecha_emision}', '{$fecha_vencimiento}', {$control1}, {$control2}, '{$fechaHoraActual}', 1, '{$observacion}', 1)";
			// echo $query;
			$exec = $lider->registrar($query, "factura_despacho_personalizada", "id_factura_despacho_perso");
			if($exec['ejecucion']==true){
				$response = "1";
		...
		if(empty($_POST)){
			...
			// }
			// $numFact = 0;
			// if($numero_factura>$numero_nota){
			// 	$numFact = $numero_factura;
			...
			$querys2 = "SELECT MAX(numero_control2) FROM factura_despacho_personalizada WHERE estatus = 1";
			...
			$query = "SELECT * FROM pedidos, factura_despacho_personalizada WHERE pedidos.id_pedido = factura_despacho_personalizada.id_pedido and pedidos.id_despacho = $id_despacho and factura_despacho_personalizada.estatus = 1";
