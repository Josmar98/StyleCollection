...
if(!empty($_POST)){
	if(!empty($_POST['cantidad_elementos']) && !empty($_POST['stock']) && !empty($_POST['inventario']) && !empty($_POST['tipos']) && !empty($_POST['total']) ){
		...
		for ($i=0; $i < $cantidad_elementos; $i++) {
			...
			$query = "INSERT INTO coleccion_modificada_perso (id_coleccion_modificada_perso, id_campana, id_despacho, id_factura_perso, codigo_identificador, stock, tipo_inventario, id_inventario, precio_venta, total_venta, estatus) VALUES (DEFAULT, {$id_campana}, {$id_despacho}, {$id_factura}, '{$codigo}', {$stocks[$i]}, '{$tipos[$i]}', {$inventarios[$i]}, {$precio}, {$totales[$i]}, 1);";
			// echo "<br>".$query."<br>";
			$exec = $lider->registrar($query, "coleccion_modificada_perso", "id_coleccion_modificada_perso");
...
if(!empty($_GET['delete'])){
	...
	$query = "UPDATE coleccion_modificada_perso SET estatus=0 WHERE id_coleccion_modificada_perso={$id_delete}";
...
if(!empty($_GET['cerrar'])){
	...
	$id_fact = $_GET['id'];
	$query = "UPDATE factura_despacho_personalizada SET estado_factura=0 WHERE id_factura_despacho_perso={$id_fact}";
	$borrar = $lider->modificar($query);
	// $borrar=['ejecucion'=>true];
	$errores = 0;
	if($borrar['ejecucion']==true){
		$notas = $lider->consultarQuery("SELECT * FROM factura_despacho_personalizada, pedidos WHERE factura_despacho_personalizada.id_pedido=pedidos.id_pedido and factura_despacho_personalizada.id_factura_despacho_perso={$id_fact}");
		...
		$mostrarListaNotas=$_SESSION['mostrarFacturaFinalPerso'.$_GET['id']];
		$numero_documento = $notas[0]['numero_factura'];
		...
		$_SESSION['mostrarFacturaFinalPerso'.$_GET['id']]=[];
		// Aqui voy a registrar las colecciones como respaldo
		$listaColecciones = $_SESSION['listaColeccionesPerso'.$_GET['id']];
		...
		$_SESSION['listaColeccionesPerso'.$_GET['id']]=[];
	}else{
		$errores++;
	}
...
if(!empty($_GET['abrir'])){
	$id_fact = $_GET['id'];
	$query = "UPDATE factura_despacho_personalizada SET estado_factura=1 WHERE id_factura_despacho_perso={$id_fact}";
	$borrar = $lider->modificar($query);
	$errores = 0;
	if($borrar['ejecucion']==true){
		$notas = $lider->consultarQuery("SELECT * FROM factura_despacho_personalizada, pedidos WHERE factura_despacho_personalizada.id_pedido=pedidos.id_pedido and factura_despacho_personalizada.id_factura_despacho_perso={$id_fact}");
