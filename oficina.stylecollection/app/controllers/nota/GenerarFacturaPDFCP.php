...
// }
	...
	$facturados = $lider->consultarQuery("SELECT * FROM operaciones WHERE operaciones.id_factura={$id_nota} and operaciones.modulo_factura='{$nameModuloFactu}' ORDER BY operaciones.id_operacion ASC;");
			...
			foreach ($inventario as $inv) {
				if(!empty($inv['elemento'])){
					...
					if(!empty($mostrarListaNotas[$codFacts])){
						...
						$mostrarListaNotas[$codFacts]['precio_nota']=$facts['precio_nota'];
					}else{
	...
	$mostrarNotasDefinitiva=[];
	foreach ($mostrarListaNotas as $key) {
		if($key['cantidad']>0){
			$mostrarNotasDefinitiva[count($mostrarNotasDefinitiva)]=$key;
		}
	}
...
$colecciones = $mostrarNotasDefinitiva;
...
<div class='row col-xs-12' style='padding:0;margin:0;'>
	<div class='col-xs-12'  style='padding-left:50px;padding-right:20px;width:100%;'>
		...
		$numPage=1;
		$colorFuente="#ED2A77";
		$colorFuente="#FFFFFF00";
		
		$font = "times new roman";
			$info .= "<table class='' style='text-align:center;width:100%;border-bottom:1px solid ".$colorFuente.";color:".$colorFuente.";opacity:0;'>
				<tr>
					<td style='width:25%;text-align:left;opacity:0;'>
						<img src='public/assets/img/logoTipo2.png' style='width:17em;opacity:0;'>
					</td>
					
					<td style='width:50%;margin-bottom:0;padding-bottom:0;opacity:0;'>
						<p style='width:100%;font-size:1.7em;margin-bottom:0;transform:scaleY(1.5);opacity:0;'>
							<b style='font-family:".$font." !important;'>INVERSIONES STYLECOLLECTION, CA</b>
						</p>
						<p style='font-size:0.9em;margin-bottom:0;padding-bottom:0;opacity:0;'>
							<b>Rif.:J408497786</b>
						</p>
						<p style='font-size:1em;margin-bottom:0;padding-bottom:0;opacity:0;'>
							<b>
					...
					<td style='width:25%;text-align:left;padding-left:10px;'>
						<b style='color:".$colorFuente.";font-size:1.2em;opacity:0;'>
							FORMA LIBRE
							<br>
							Nro DE CONTROL
						</b>
						<br>
						<b style='color:".$colorFuente.";margin-left:-2em;font-size:1.8em;opacity:0;'>
			...
			<img src='public/assets/img/iconoSinFondo.png' style='width:25em;height:30em;position:absolute;z-index:-10%;top:".$topPorcenImg."%;left:33%;opacity:0;'>
			<div class='row'>
				<div class='col-xs-12'>
					";
						...
						foreach($colecciones as $cols){
							if(!empty($cols['descripcion'])){
								if($numero<=$numLim){
									// print_r($cols);
									// echo "<br><br>";
									$cols['precio']=$cols['precio_nota'];
									$cols['total']=($cols['precio_nota']*$cols['cantidad']);
									$info.="
									<tr style=''>
									...
									<td class='celtitleL' style='width:100%;'>
										...
										<small style='font-size:0.85em;'>";
											...
											if($notaentrega['observaciones']!=""){
												$info.= $notaentrega['observaciones'];
											}else{
