...
if($estado_campana=="1"){
	...
	if (!empty($_POST)) {
		...
		$max = count($opts);
		$query = "INSERT INTO notasentrega (id_nota_entrega, id_cliente, id_campana, direccion_emision, lugar_emision, fecha_emision, numero_nota_entrega, nombreanalista, estatus) VALUES (DEFAULT, {$id_lider}, {$id_campana}, '{$direccion}', '{$lugar}', '{$fecha}', {$num}, '{$nameAnalista}', 1)";
		// echo $query."<br>";
		...
		if(!empty($_GET['admin']) && !empty($_GET['lider']) && ($_SESSION['nombre_rol']=="Superusuario" || $_SESSION['nombre_rol']=="Administrador" || $_SESSION['nombre_rol']=="Analista Supervisor" || $_SESSION['nombre_rol']=="Administrativo" || $_SESSION['nombre_rol']=="Analista")){
			...
			// $planesCol = $lider->consultarQuery("SELECT * FROM confignotaentrega, planes, planes_campana, tipos_colecciones, pedidos WHERE confignotaentrega.id_plan = planes.id_plan and confignotaentrega.opcion = 1 and planes.id_plan = planes_campana.id_plan and planes_campana.id_plan_campana = tipos_colecciones.id_plan_campana and pedidos.id_pedido = tipos_colecciones.id_pedido and pedidos.id_despacho = {$id_despacho} and planes_campana.id_campana = {$id_campana} and confignotaentrega.id_campana = {$id_campana} and tipos_colecciones.id_pedido = {$pedido['id_pedido']} and pedidos.id_cliente = {$id}");
