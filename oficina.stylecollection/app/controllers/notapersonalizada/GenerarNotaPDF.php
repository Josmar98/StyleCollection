...
// }
	...
	$limitesPages = [];
	if(($resto-$extrem)>=0){
		$limitesPages[count($limitesPages)]=$extrem;
	}else{
		$limitesPages[count($limitesPages)]=$resto;
	}
	while(($resto-$extrem)>=0){
		$veces++;
		$resto-=$extrem;
		$limitesPages[count($limitesPages)]=$resto;
	}
	$newMostrarListaNotas = [];
	foreach ($mostrarListaNotas as $key) {
		$newMostrarListaNotas[count($newMostrarListaNotas)]=$key;
	}
	// print_r($limitesPages);
	// echo "<br>";
	// echo "veces: ".$veces;
			...
			<body>
				...
				<div class='row' style='padding:0;margin:0;'>
					<div class='col-xs-12'  style='width:100%;'>";
						$page=1;
						$inicio = 0;
						for ($numPage=0; $numPage < $veces; $numPage++) { 
							$info .= "
							<div class='box-body' style='background:;'>
								...
								<table class='table table-bordered' style='border:none;'>
									...
									<tr>
										...
										<td colspan='2'>
											Nro. Tlf: 
											<span style='margin-left:10px;margin-right:10px;'></span>
											".separateDatosCuentaTel($persona['telefono'])." ";
											if(strlen($persona['telefono2'])>5){
												$info .= " / ".separateDatosCuentaTel($persona['telefono2']);
											}
											$info .= "
										</td>
									</tr>
									<tr>
										<td colspan='5'>
											Observaciones:
											<span style='margin-left:10px;margin-right:10px;font-size:0.8em;'>
												".$notaentrega['observacion']."
											</span>
										</td>
								...
								<table class='table table-bordered text-left' style='width:100%;'>
									...
									<tbody>";
										$num = 1;
										// PAGINAS
										for ($i=$inicio; $i < ($limitesPages[$numPage]+$inicio); $i++) { 
											$notaM = $newMostrarListaNotas[$i];
