			...
			if(Count($buscar)<2){
				...
				if(Count($buscar)<2){
					...
					if($id_banco==""){
						...
						$query = "INSERT INTO pagos (id_pago, id_pedido, id_campana, fecha_pago, fecha_registro, forma_pago, tipo_pago, referencia_pago, monto_pago, tasa_pago, equivalente_pago, estado, estatus) VALUES ('{$pagoID}', $id_pedido, $id_campana, '$fechaPago', '".date('Y-m-d')."', '$forma_pago', '$tipoPago', '$serial', '$monto', '$tasa', '$eqv', 'Abonado', 1)";
						// echo $query;
					}else{
						// echo $id_banco."<br>";
						if($referencia!=""){
							// echo "Hay Referencia no Serial";
							$query = "INSERT INTO pagos (id_pago, id_pedido, id_campana, id_banco, fecha_pago, fecha_registro, forma_pago, tipo_pago, referencia_pago, monto_pago, tasa_pago, equivalente_pago, estatus) VALUES ('{$pagoID}', $id_pedido, $id_campana, $id_banco, '$fechaPago', '".date('Y-m-d')."', '$forma_pago', '$tipoPago', '$referencia', '$monto', '$tasa', '$eqv', 1)";
						}else if($serial!=""){
							// echo "Hay Serial no Referencia";
							$query = "INSERT INTO pagos (id_pago, id_pedido, id_campana, id_banco, fecha_pago, fecha_registro, forma_pago, tipo_pago, referencia_pago, monto_pago, tasa_pago, equivalente_pago, estatus) VALUES ('{$pagoID}', $id_pedido, $id_campana, $id_banco, '$fechaPago', '".date('Y-m-d')."', '$forma_pago', '$tipoPago', '$serial', '$monto', '$tasa', '$eqv', 1)";
						}
					}
					...
					if($exec['ejecucion']==true){
						...
						if(!empty($modulo) && !empty($accion)){
							$fecha = date('Y-m-d');
							$hora = date('H:i:a');
							$query = "INSERT INTO bitacora (id_bitacora, id_usuario, modulo, accion, fecha, hora) VALUES ('{$pagoID}', {$_SESSION['id_usuario']}, 'Pago', 'Registrar', '{$fecha}', '{$hora}')";
