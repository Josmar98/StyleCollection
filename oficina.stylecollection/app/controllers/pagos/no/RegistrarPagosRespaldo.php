...
// if($_SESSION['nombre_rol']=="Administrador" || $_SESSION['nombre_rol']=="Superusuario"){
		...
		if(!empty($_POST['valForma']) && !empty($_POST['fechaPago']) && !empty($_POST['tipoPago']) ){
			...
			if(Count($buscar)<2){
				...
				// echo $id_pedido;
				// $count = $lider->consultarQuery("SELECT count(id_pago) FROM pagos WHERE id_pedido = {$id_pedido}");
				// $numero_pago = $count[0]['count(id_pago)'];
				// $numero_pago+=1;
				
				// $count = $lider->consultarQuery("SELECT * FROM pagos WHERE id_pedido = {$id_pedido}");
				// $numero_pago = count($count);
