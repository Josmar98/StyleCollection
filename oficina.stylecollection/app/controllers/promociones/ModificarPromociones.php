...
if(!empty($_POST['cantidad']) && empty($_POST['validarData'])){
  ...
  if(count($pedidos)>1){
    ...
    $query = "UPDATE promociones SET id_promocion={$id_promocion}, cantidad_solicitada_promocion={$cantidad} WHERE id_promociones={$id}";
    // echo $query;
  ...
  if(count($promocionEditar)>1){
    $promocionEditar = $promocionEditar[0];
    if(!empty($action)){
