...
if($amReportesC == 1){
    ...
    $facturado = [];
    $facturado=$_SESSION['GenerarPremiosAlcanzadosResumen'];
    ...
    //                     $verifConcepto=0;
    //                     foreach ($facturado[$code]['conceptos'] as $conceptss) {
    //                         $comparar1=mb_strtolower($key['concepto_factura']);
    //                         $comparar2=mb_strtolower("premios de");
    //                         if(strlen(strpos($comparar1, $comparar2))>0){
    //                             $comparar1=substr($comparar1, strlen(mb_strtolower("premios de"))+1);
    //                         }
    //                         if(strlen(strpos($comparar1, $comparar2))>0){
    //                             $comparar1=substr($comparar1, strlen(mb_strtolower("premios de"))+1);
    //                         }
    ...
    //                             $comparar1=substr($comparar1, strlen(mb_strtolower("premios de"))+1);
    //                         }
    //                         $key['concepto_factura']=$comparar1;
    //                         if($conceptss==$key['concepto_factura']){
    //                             $verifConcepto++;
    //                         }
    //                     }
    //                     if($verifConcepto==0){
    //                         $facturado[$code]['concepto'].=", ".substr($key['concepto_factura'],11);
    //                         $facturado[$code]['conceptos'][count($facturado[$code]['conceptos'])]=substr($key['concepto_factura'],11);
    //                     }
    //                 }else{
    //                     $comparar1=mb_strtolower($key['concepto_factura']);
    //                     $comparar2=mb_strtolower("premios de");
    ...
    // }
		...
		// }
			...
			<head>
				...
				<title>Resumen de Premios Alcanzados de Campaña ".$numeroCampana."/".$anioCampana." - StyleCollection</title>
            ...
            for ($numPage=1; $numPage <= $maxPage; $numPage++) { 
                ...
                <div class='row' style='padding:0;margin:0;'>
                    ...
                    ";		
                        $info .= "
                            <h4 style='width:100%;border:2px solid #ccc;text-align:center;padding:3px;margin-botton:30px;'>Resumen - Premios Alcanzados</h4>
                            <br>
                                ...
                                <tbody style='font-size:0.9em;'> ";
                                    foreach ($facturarr as $data) {
                                        ...
                                        <tr style='padding:0;margin:-5px;'>
                                            ...
                                            <td style='text-align:left !important;'>{$data['descripcion']}</td>
                                            <td style='text-align:left !important;'>";
                                            $info .= $data['detalles'];
                                            if($data['detalles2']!=""){ 
                                                $info .= " <small style='font-size:0.8em;'>(".$data['detalles2'].")</small>";
                                            }
                                            $info .= "</td>
                                        </tr>
                                        ";
			...
			$dompdf->render();
			$dompdf->stream("Resumen de Premios Alcanzados de Campaña {$numeroCampana}-{$anioCampana} - StyleCollection", array("Attachment" => false));
			// echo $info;
}else{
