  ...
  if(isset($_GET['C']) || isset($_GET['ID'])){
    ...
    foreach ($catalogo as $key) {
      if(!empty($key['id_catalogo'])){
        ...
        $newDataAcum[$index]['cantidad_asignado'] = 0;
        $newDataAcum[$index]['cantidad_noasignado'] = 0;
      ...
      if($permitido=="1"):
		...
		if($accesoBloqueo=="1"){
			...
			<head>
				...
				<title>Premios canjeados general de líderes";
				if(!empty($_GET['id'])){
			...
			<div class='row' style='padding:0;margin:0;'>
				<div class='col-xs-12'  style='width:100%;'>
						...
						<h3 style='text-align:right;float:right;'>
							...
							$info .= "</small></h3>
						<h2 style='font-size:1.9em;'> Premios canjeados general de líderes";
								if(!empty($_GET['id'])){
				...
				";		
										...
										if($permitido=="1"):
											foreach ($newData2 as $infos): if (!empty($infos['id_cliente'])): if($data['id_cliente']==$infos['id_cliente']):
												...
												$info .= "<tr class='text-center'>
														...
														if(!empty($elemento['id_cliente'])){
															if($elemento['id_cliente']==$data['id_cliente']){
																if(empty($newData[$elemento['id_catalogo']]['cantidad_canjeo'])){
																	...
																	$newData[$elemento['id_catalogo']]['cantidad_canjeo_asignado']=0;
																	$newData[$elemento['id_catalogo']]['cantidad_canjeo_noasignado']=0;
																}
																...
																if($elemento['estado_canjeo']=="Asignado"){
																	$newData[$elemento['id_catalogo']]['id_catalogo_asignado'] = $elemento['id_catalogo'];
																	$newData[$elemento['id_catalogo']]['nombre_catalogo_asignado'] = $elemento['nombre_catalogo'];
																	$newData[$elemento['id_catalogo']]['cantidad_gemas_asignado'] = $elemento['cantidad_gemas'];
																	$newData[$elemento['id_catalogo']]['marca_catalogo_asignado'] = $elemento['marca_catalogo'];
																	$newData[$elemento['id_catalogo']]['cantidad_canjeo_asignado']++;
																	$newDataAcum[$elemento['id_catalogo']]['cantidad_asignado']++;
																}
																if($elemento['estado_canjeo']!="Asignado"){
																	$newData[$elemento['id_catalogo']]['id_catalogo_noasignado'] = $elemento['id_catalogo'];
																	$newData[$elemento['id_catalogo']]['nombre_catalogo_noasignado'] = $elemento['nombre_catalogo'];
																	$newData[$elemento['id_catalogo']]['cantidad_gemas_noasignado'] = $elemento['cantidad_gemas'];
																	$newData[$elemento['id_catalogo']]['marca_catalogo_noasignado'] = $elemento['marca_catalogo'];
																	$newData[$elemento['id_catalogo']]['cantidad_canjeo_noasignado']++;
																	$newDataAcum[$elemento['id_catalogo']]['cantidad_noasignado']++;
																}
															}
														}
													}
													$info .= "<td style='width:25%;text-align:left;'>";
													...
													$info .= "<td style='width:25%;text-align:left;'>";
													foreach ($newData as $infos2) { if(!empty($infos2['id_catalogo_asignado'])){
														$info .= "(".$infos2['cantidad_canjeo_asignado'].") ".$infos2['nombre_catalogo_asignado']." ".$infos2['marca_catalogo_asignado']."<br>";
													} }
													$info .= "</td>";

													$info .= "<td style='width:25%;text-align:left;'>";
													foreach ($newData as $infos2) { if(!empty($infos2['id_catalogo_noasignado'])){
														$info .= "(".$infos2['cantidad_canjeo_noasignado'].") ".$infos2['nombre_catalogo_noasignado']." ".$infos2['marca_catalogo_noasignado']."<br>";
													} }
													$info .= "</td>";
					...
					$info .= "		<tr style='background:#efefef55;'>
			                            ...
			                            <td style='text-align:left;'>
			                              <b>";
			                              	foreach ($newDataAcum as $key) { if($key['cantidad_asignado']>0){
			                              		$info .= "(".$key['cantidad_asignado'].") ".$key['nombre']." ".$key['marca']."<br>";
			                              	} }
			                              $info .= "</b>
			                            </td>
			                            <td style='text-align:left;'>
			                              <b>";
			                              	foreach ($newDataAcum as $key) { if($key['cantidad_noasignado']>0){
			                              		$info .= "(".$key['cantidad_noasignado'].") ".$key['nombre']." ".$key['marca']."<br>";
			                              	} }
			                              $info .= "</b>
			                            </td>
			                          </tr>
			...
			";
				...
				$namedoc = "Premios canjeados general de líderes ";
				if(!empty($_GET['id'])){
