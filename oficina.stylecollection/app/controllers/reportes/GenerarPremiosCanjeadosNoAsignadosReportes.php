...
if($amReportesC == 1){
  if(isset($_GET['C']) || isset($_GET['ID'])){
    ...
    if(isset($_GET['C'])){
      if($_GET['C'] == 0){
        $listado = $lider->consultarQuery("SELECT * FROM canjeos, catalogos WHERE catalogos.id_catalogo = canjeos.id_catalogo and canjeos.estatus = 1{$queryFechas} and canjeos.estado_canjeo IS NULL");
      }
      if($_GET['C'] > 0){
        $listado = $lider->consultarQuery("SELECT * FROM canjeos, catalogos WHERE catalogos.id_catalogo = canjeos.id_catalogo and canjeos.estatus = 1{$queryFechas} and canjeos.estado_canjeo IS NULL and catalogos.cantidad_gemas = {$_GET['C']}");
      }
    }
    if(isset($_GET['ID'])){
      if($_GET['ID'] == 0){
      ...
      if($_GET['ID'] > 0){
        $listado = $lider->consultarQuery("SELECT * FROM canjeos, catalogos WHERE catalogos.id_catalogo = canjeos.id_catalogo and canjeos.estatus = 1{$queryFechas} and canjeos.estado_canjeo IS NULL and catalogos.id_catalogo = {$_GET['ID']}");
      }
      ...
      if($permitido=="1"):
		...
		if($accesoBloqueo=="1"){
			...
			<head>
				...
				<title>Premios canjeados de líderes No Asignados";
				if(!empty($_GET['id'])){
			...
			<div class='row' style='padding:0;margin:0;'>
				<div class='col-xs-12'  style='width:100%;'>
						...
						<h3 style='text-align:right;float:right;'>
							...
							$info .= "</small></h3>
						<h2 style='font-size:1.9em;'> Premios canjeados de líderes No Asignados";
								if(!empty($_GET['id'])){
			...
			";
				...
				$namedoc = "Premios canjeados de líderes No Asignados";
				if(!empty($_GET['id'])){
