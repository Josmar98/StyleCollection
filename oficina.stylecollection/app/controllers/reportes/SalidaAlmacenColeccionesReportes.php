...
if($amReportesC == 1){
  if(!empty($_POST)){
    ...
    $id_campana=$despacho['id_campana'];
    $fecha_guardado = date('Y-m-d');
    $hora_guardado = date('H:i:s');
    $name_reporte = $_GET['action'];
    $nombre_ruta = $_GET['ruta'];
    $rutaURL = $_POST['ruta'];
    // echo "id_campana: ".$id_campana."\n";
    // echo "id_despacho: ".$id_despacho."\n";
    // echo "name_reporte: ".$name_reporte."\n";
    // echo "fecha_guardado: ".$fecha_guardado."\n";
    // echo "hora_guardado: ".$hora_guardado."\n";
    // echo "nombre_ruta: ".$nombre_ruta."\n";
    // // echo "rutaURL LENGTH: ".strlen($rutaURL)."\n";
    // echo "rutaURL: ".$rutaURL."\n";
    $buscar = $lider->consultarQuery("SELECT * FROM historial_busqueda_reportes WHERE rutaURL='{$rutaURL}'");
    // print_r($buscar);
    if(count($buscar)<2){
        $query="INSERT INTO historial_busqueda_reportes (id_historial_busqueda, id_campana, id_despacho, name_reporte, fecha_guardado, hora_guardado, nombre_ruta, rutaURL, estatus) VALUES (DEFAULT, {$id_campana}, {$id_despacho}, '{$name_reporte}', '{$fecha_guardado}', '{$hora_guardado}', '{$nombre_ruta}', '{$rutaURL}', 1)";
        $exec = $lider->registrar($query, "historial_busqueda_reportes", "id_historial_busqueda");
    ...
    if(!empty($_GET['P'])){
      ...
      $index = 0;
      $zonasClientes = [];
      for ($i=0; $i < $_GET['cantidad_elementos']; $i++) { 
        if($i>0){
          if($index < count($clientesss)){
            $filterCLientes.=", "; 
          }
        }
        $filterCLientes.=$clientesss[$i];
        $zonasClientes[$i]['id_cliente']=$clientesss[$i];
        $zonasClientes[$i]['zona']=$zonasss[$i];
      }
      // print_r($zonasClientes);
      ...
      foreach ($promosNames as $key) {
        if(!empty($key['id_promocion'])){
          ...
          // $namePromo = substr($key['nombre_promocion'], strlen($key['nombre_promocioninv'])+1);
          $namePromo = substr($key['nombre_promocion'], strpos($key['nombre_promocion'], "(")+1);
