...
if(!empty($_POST['validarData'])){
  ...
  $query = "SELECT * FROM servicios WHERE id_cliente = {$id_user} and id_promocion = {$id_promo} and id_despacho = {$id_despacho}";
...
if(!empty($_POST['cantidad']) && empty($_POST['validarData'])){
  ...
  if(!empty($_POST['clientes']) && !empty($_GET['admin']) && ($_SESSION['nombre_rol'] == "Administrador" || $_SESSION['nombre_rol'] == "Superusuario" || $_SESSION['nombre_rol'] == "Analista Supervisor" || $_SESSION['nombre_rol'] == "Analista" )){
    $id_users = $_POST['clientes'];
  }else{
    $id_users[0] = $_SESSION['id_cliente'];
  }
  // die();
  ...
  foreach($id_users as $id_user){
    ...
    if(count($pedidos)>1){
      ...
      if(count($buscar)>1){
        if($buscar[0]['cantidad_servicio'] > $cantidad){
          $cantidad = $buscar[0]['cantidad_servicio'];
        }
        $query = "UPDATE servicios SET cantidad_servicio={$cantidad}, fecha_servicio='{$fecha_solicitud}', hora_servicio='{$hora_solicitud}' WHERE id_servicios={$buscar[0]['id_servicios']}";
        // echo "<br>".$query."<br><br>";
        $exec = $lider->modificar($query);
      }else{
        // $response = "9";
        $query = "INSERT INTO servicios (id_servicios, id_cliente, id_pedido, id_servicio, id_despacho, id_campana, cantidad_servicio, fecha_servicio, hora_servicio, estatus) VALUES (DEFAULT, {$id_user}, {$id_pedido}, {$id_servicio}, {$id_despacho}, {$id_campana}, {$cantidad}, '{$fecha_solicitud}', '{$hora_solicitud}', 1)";
        // echo "<br>".$query."<br><br>";
        $exec = $lider->registrar($query, "servicios", "id_servicios");
      }
  ...
  if(!empty($_GET['admin']) && ($_SESSION['nombre_rol'] == "Administrador" || $_SESSION['nombre_rol'] == "Superusuario" || $_SESSION['nombre_rol'] == "Analista Supervisor" || $_SESSION['nombre_rol'] == "Analista") ){
    ...
    $clientesConPromociones = $lider->consultarQuery("SELECT clientes.id_cliente, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, cedula, sexo, fecha_nacimiento, telefono, correo, clientes.estatus FROM clientes, pedidos, servicios WHERE clientes.id_cliente = pedidos.id_cliente and pedidos.id_pedido = servicios.id_pedido and clientes.id_cliente = servicios.id_cliente and servicios.id_despacho = {$id_despacho} and servicios.estatus = 1");
    if(!empty($_GET['lider'])){
      $id_user = $_GET['lider'];
    }else{
      $id_user = 0;
  ...
  if(!empty($_GET['all'])){
    $pedidosFull = $lider->consultarQuery("SELECT * FROM pedidos, clientes WHERE pedidos.id_cliente = clientes.id_cliente and pedidos.id_despacho = $id_despacho");
  }
  if(count($pedidos)>1){
    $pedido = $pedidos[0];
  }else{
