<?php 
	...
	if(is_file('config/database3.php')){
		require_once'config/database3.php';
	}
	else if(is_file('../config/database3.php')){
		require_once'../config/database3.php';
	}
	...
	class Modelss extends Conexion3{
		...
		public function __construct(){Conexion3::realizarConexion();}

		public function registrar($query, $table, $id){
			if(Conexion3::getEstatusConexion()){
				try{
					$strExe = Conexion3::prepare($query);
					$strExe->execute();
					$this->id = Conexion3::getLastId($table,$id);
