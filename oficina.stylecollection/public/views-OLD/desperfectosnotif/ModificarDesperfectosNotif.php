                  ...
                  <div class="row">
                    ...
                    <?php foreach ($colecciones as $collss): if(!empty($collss['id_producto'])):?>
                      ...
                      <div class="col-xs-12 col-sm-6 col-md-4">
                        ...
                        <input type="number" <?php foreach ($desperfectos as $keys){ if(!empty($keys['id_producto'])){ if ($keys['id_producto']==$collss['id_producto']){ ?> value="<?=$keys['cantidad_desperfectos']?>" <?php } } } ?> value="0" min="0" max="<?=$collss['cantidad_productos']*$pedido['cantidad_aprobado'] ?>" id="<?php echo $collss['id_producto'] ?>" class="form-control cantidades" name="cantidad_desperfectos[]">
                      </div>
                      <div class="col-xs-12 col-sm-6 col-md-4">
                        <label for="descripcion<?=$collss['id_producto']; ?>">Especifique el desperfecto en el/los producto/s</label>
                        <textarea class="form-control" name="descripcion_desperfectos[]" id="descripcion<?=$collss['id_producto']; ?>" style="min-width:100%;max-width:100%;min-height:35px;max-height:70px;" maxlength="250" placeholder="Describa a detalle el desperfecto encontrado"><?php foreach ($desperfectos as $keys){ if(!empty($keys['id_producto'])){ if ($keys['id_producto']==$collss['id_producto']){echo $keys['descripcion_desperfectos']; } } } ?></textarea>
                      </div>
