              ...
              <h3 class="box-title">Modificar <?php echo "".$modulo; ?></h3>
                  ...
                  <form action="" method="post" role="form" class="form_register">
                    ...
                    <div class="box-body">
                                  ...
                                  foreach ($mercancia as $data){
                                    if(!empty($data['id_mercancia'])){
                                      ...
                                      <tr class="elementTR">
                                        ...
                                        <td style="width:20%">
                                          <input type="number" step="0.10" min="0" name="precios[]" value="<?=$valuePrecio; ?>" class="precio_productos preciosidm<?=$data['id_mercancia'] ?> form-control" style="width:50%;padding:0px 5px" <?php if($valueIdProduct){ echo "readonly"; } ?>>
                                          <input type="hidden" name="precioid[]" value="m<?=$data['id_mercancia'] ?>">
                                        </td>
                                        <td style="width:20%">
                                          <input type="number" min="0" name="cantidad_productos[]" value="<?=$valueCantidad; ?>" class="cantidad_productos cantidadm<?=$data['id_mercancia'] ?> form-control" style="width:50%;padding:0px 5px"  <?php if($valueIdProduct){ echo "readonly"; } ?>>
                                          <input type="hidden" name="elementosid[]" value="m<?=$data['id_mercancia'] ?>">
                                        </td>
                                        <td style="width:10%">
                                          <input type="checkbox" class="cheking check<?=$data['id_mercancia'] ?>" value="m<?=$data['id_mercancia'] ?>" name="cheking[]" <?php if($valueIdProduct){ echo "checked"; } ?> >
                                        </td>
                                      </tr>
                                      <?php
                                    }
                        ...
                        <div class="row">
                          <?php 
                            $limitePagos = count($cantidadPagosDespachosFild)-1;
                            $indexOfPagos=1;
                          ?>
                          <?php
                            ...
                            <div class="form-group col-sm-6">
                              ...
                              <div class="input-group">
                                ...
                                <input type="number" step="0.1" class="form-control" min="0" id="precio_<?=$cvPagos['id']; ?>" name="preciosPagos[precio_<?=$cvPagos['id']; ?>]" maxlength="30" placeholder="Precio de <?=mb_strtolower($cvPagos['name']); ?> la coleccion" 
                                  <?php
                                    if($indexOfPagos<=$limitePagos){
                                    foreach ($pagos_despacho as $pagosDesp) {
                                      if(!empty($pagosDesp['tipo_pago_despacho'])){
                                        if($cvPagos['name']==$pagosDesp['tipo_pago_despacho']){
                                          echo "value='".$pagosDesp['pago_precio_coleccion']."' readonly";
                                        }
                                      }
                                    }
                                    }else{
                                      echo "value='{$precioPagoSec}'";
                                    }
                                  ?>
