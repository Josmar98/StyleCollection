                  ...
                  <div class="row">
                    <div class="form-group col-xs-12">
                      ...
                      <select class="form-control select2 selectLider" id="lider" name="lider" style="width:100%;">
                        <option <?php if (!empty($_GET['lider'])): if($lideres['id_cliente']==$_GET['lider']): ?> selected="selected" <?php endif; endif; ?> value="<?=$lideres['id_cliente']?>"><?=$lideres['primer_nombre']." ".$lideres['primer_apellido']." ".$lideres['cedula']?></option>
                      </select>
                    </div>
                  </div>
                </div>
                    ...
                    ?>
                      <div class="row">
                        <div class="form-group col-xs-12 col-sm-6 col-md-6">
                          ...
                          <input type="date" class="form-control fechaPago" min="<?=$limiteFechaMinimo?>" value="<?=$eficoin['fecha_pago']; ?>" readonly name="fechaPago" id="fechaPago" max="<?=date('Y-m-d')?>">
                          <span id="error_fechaPago" class="errors"></span>
                        </div>
                        <div class="form-group col-xs-12 col-sm-6 col-md-6">
                          <label for="tasa_eficoin">Fecha de Pago</label>
                          <input type="number" class="form-control tasa_eficoin" value="<?=$detalleEficoin['tasa_eficoin']; ?>" readonly name="tasa_eficoin" id="tasa_eficoin">
                          <span id="error_tasa_eficoin" class="errors"></span>
                        </div>
                      </div>
                      ...
                      <div class="form-group col-xs-12 col-sm-6 col-md-6">
                        ...
                        <select class="form-control select2" id="tipoPago" name="tipoPago" style="width:100%;">
                            ...
                            foreach ($pagos_despacho as $pagosD){
                                ...
                                ?>
                                    <option <?php if($pagosD['tipo_pago_despacho']==$detalleEficoin['tipo_pago']){ echo "selected"; } ?>><?=$pagosD['tipo_pago_despacho']; ?></option>
                                <?php
                                }
                            }
                            foreach ($promociones as $promoPagos) {
                                ...
                                if($promoPagos['cantidad_aprobada_promocion']>0){
                                    ?>
                                    <option <?php if($pagosD['tipo_pago_despacho']==$detalleEficoin['tipo_pago']){ echo "selected"; } ?>><?=$promoPagos['nombre_promocion'] ?></option>
                                    <?php
                                }
                                }
                            }
                      ...
                      <input type="hidden" name="id_detalle_eficoin" value="<?=$detalleEficoin['id_detalle_eficoin']; ?>">
                      <div class="form-group col-xs-12 col-sm-6 col-md-6">
                        <?php 
                          $nameLabelSerial = "";
                          $clasemd = "";
                          if($detalleEficoin['id_banco']!=""){
                            $clasemd = "col-md-4";
                            $nameLabelSerial = "Numero de Ticket de deposito";
                          }else{
                            $clasemd = "col-md-6";
                            $nameLabelSerial = "Serial de billete en dolar";
                          }
                        ?>
                        <label for="serial"><?=$nameLabelSerial; ?></label>
                        <input type="text" class="form-control" id="serial" value="<?=$detalleEficoin['referencia_pago']; ?>" name="serial">
                        <span id="error_serial" class="errors"></span>
                      </div>
                    </div>
                    <div class="row">
                      <?php if($detalleEficoin['id_banco']!=""){ ?>
                      <div class="form-group col-xs-12 col-sm-6 col-md-4">
                        ...
                        <select class="form-control select2 banco" style="width:100%;" name="banco" id="banco">
                          <!-- <option value=""></option> -->
                          <?php 
                            foreach ($bancosDepositos as $bank) { if(!empty($bank['id_banco'])){
                              if($bank['id_banco']==$detalleEficoin['id_banco']){
                                ?>
                      ...
                      <?php } ?>
                      <div class="form-group col-xs-12 col-sm-6 <?=$clasemd; ?>">
                        <label for="equivalente">Equivalente</label>
                        <div class="input-group">
                          <span class="input-group-addon" style="background:#EEE">$</span> 
                          <input type="number" step="0.01" class="form-control equivalente montoDinero" readonly value="<?=$detalleEficoin['equivalente_pago']; ?>" id="equivalente" class="equivalente" name="equivalente">
                        </div>
                        <span id="error_equivalente" class="errors"></span>
                      </div>
                      <div class="form-group col-xs-12 col-sm-6 <?=$clasemd; ?>">
                        <label for="equivalente_total">Equivalente Total</label>
                        <div class="input-group">
                          <span class="input-group-addon" style="background:#EEE">$</span> 
                          <input type="number" step="0.01" class="form-control equivalente_total montoDinero" readonly value="<?=$detalleEficoin['equivalente_pago_total']; ?>" id="equivalente_total" class="equivalente_total" name="equivalente_total">
                        </div>
                        <span id="error_equivalente_total" class="errors"></span>
                      </div>
                    </div>
                </div>
              <!-- /.box-body -->
