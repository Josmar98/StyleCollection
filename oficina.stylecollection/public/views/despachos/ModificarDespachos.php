          ...
          <div class="box">
            <div class="box-header with-border">
              ...
              <div style="width:100%;text-align:right;">
                <a href="?<?=$menu."&route=Despachos&action=Modificar&id=".$id?>" style="border:0;background:none;color:#04a7c9">Editar opcion de incial y cantidad de pagos <span class="fa fa-wrench"></span></a>
              </div>
              <br>
              <div style="width:100%;text-align:right;">
                <!-- <a href="?<?=$menu."&route=Despachos&action=ModificarFechas&id=".$id."&opInicial=".$_GET['opInicial']."&opOpt=".$_GET['opOpt']."&cantPagos=".$_GET['cantPagos'];?>" style="border:0;background:none;color:#04a7c9">Editar Fechas de Pedido <span class="fa fa-wrench"></span></a> -->
              </div>
              <?php } ?>
            ...
            <?php if ( empty($_GET['opInicial']) && empty($_GET['cantPagos']) ): ?>
              <form action="" method="get" role="form" class="form_register">
                    ...
                    <div class="row">
                      ...
                      <div class="form-group col-xs-12 col-sm-6">
                        ...
                        <select class="form-control select2" id="" name="opOpt" style="width:100%;">
                          <option value="Y" <?php if($despacho['opcionOpcionalInicial']=="Y"){ echo "selected='selected'"; } ?> >SI</option>
                          <option value="N" <?php if($despacho['opcionOpcionalInicial']=="N"){ echo "selected='selected'"; } ?> >NO</option>
                        </select>
                    ...
                    <div class="row">
                      <div class="form-group col-xs-12 col-sm-6">
                        ...
                        <select class="form-control select2" id="" name="opOblig" style="width:100%;">
                          <option value="Y" <?php if($despacho['opcionInicialObligatorio']=="Y"){ echo "selected='selected'"; } ?> >SI</option>
                          <option value="N" <?php if($despacho['opcionInicialObligatorio']=="N"){ echo "selected='selected'"; } ?> >NO</option>
                        </select>
                ...
                <div class="box-footer">
                  ...
                  <a style="margin-left:5%" href="?<?php echo $menu ?>&route=<?php echo $url ?>&action=Modificar&id=<?=$_GET['id']; ?>&opInicial=<?=$despacho['opcion_inicial']; ?>&opOpt=<?=$despacho['opcionOpcionalInicial'];?>&opOblig=<?=$despacho['opcionInicialObligatorio'];?>&cantPagos=<?=$despacho['cantidad_pagos']; ?>" class="btn btn-default">Cancelar</a>
                </div>
              </form>
            <?php 
                    ...
                    <div class="box-body">
                      <div class="row">
                        ...
                        <div class="form-group col-xs-12 col-sm-6">
                          ...
                          <input type="text" class="form-control" id="nombre_despacho" name="nombre_despacho" value="<?php echo $despacho['nombre_despacho'] ?>">
                        </div>
                        ...
                        <div class="form-group col-xs-12 col-sm-6">
                          ...
                          <select class="form-control select2" id="plan_seleccion" name="plan_seleccion" style="width:100%;">
                            ...
                            <option 
                              value="<?=$cvPagos['id']; ?>"
                              <?php
                                foreach ($pagos_despacho as $key){ if (!empty($key['id_despacho'])){ 
                                  if($key['tipo_pago_despacho']==$cvPagos['name']){ 
                              ...
                              <?php 
                                ...
                                foreach ($productos as $data){
                                  ...
                                  <tr class="elementTR">
                                    ...
                                    <td style="width:20%">
                                      ...
                                      <input type="number" step="0.10" min="0" name="precios[]" <?php foreach ($colecciones as $key){if(!empty($key['id_coleccion'])){if( ($data['id_producto'] == $key['id_producto']) && ($key['tipo_inventario_coleccion']=="Productos") ){ ?> value="<?php echo $key['precio_producto']; ?>" readonly="" <?php }}} ?> class="precio_productos preciosid<?php echo $data['id_producto'] ?> form-control" style="width:50%;padding:0px 5px">
                                    ...
                                    <td style="width:20%">
                                      <input type="number" min="0" name="cantidad_productos[]" class="cantidad_productos cantidad<?php echo $data['id_producto'] ?> form-control" style="width:50%;padding:0px 5px" <?php foreach ($colecciones as $key){ if(!empty($key['id_coleccion'])){ if( ($data['id_producto'] == $key['id_producto']) && ($key['tipo_inventario_coleccion']=="Productos") ){ ?> value="<?php echo $key['cantidad_productos'] ?>" readonly="" <?php }else{ ?>  <?php }}} ?> >
                                      <input type="hidden" name="elementosid[]" value="<?php echo $data['id_producto'] ?>">
                                    </td>
                                    <td style="width:10%">
                                      <input type="checkbox" class="cheking check<?php echo $data['id_producto'] ?>" value="<?php echo $data['id_producto'] ?>" name="cheking[]" <?php foreach ($colecciones as $key){if(!empty($key['id_coleccion'])){ if( ($data['id_producto'] == $key['id_producto']) && ($key['tipo_inventario_coleccion']=="Productos") ){ echo "checked=''"; }}} ?>>
                                ...
                                foreach ($mercancia as $data){
                                  ...
                                  <tr class="elementTR">
                                    ...
                                    <td style="width:20%">
                                      <!-- <input type="number" min="0" class="preciosid<?php echo $data['id_mercancia'] ?>" name="precios[]" value="<?php echo $data['precio_producto'] ?>"> -->
                                      <input type="number" step="0.10" min="0" name="precios[]" <?php foreach ($colecciones as $key){if(!empty($key['id_coleccion'])){ if( ($data['id_mercancia'] == $key['id_producto']) && ($key['tipo_inventario_coleccion']=="Mercancia") ){ ?> value="<?=$key['precio_producto']; ?>" readonly="" <?php }}} ?> class="precio_productos preciosidm<?=$data['id_mercancia'] ?> form-control" style="width:50%;padding:0px 5px">
                                    ...
                                    <td style="width:20%">
                                      <input type="number" min="0" name="cantidad_productos[]" class="cantidad_productos cantidadm<?=$data['id_mercancia']; ?> form-control" style="width:50%;padding:0px 5px" <?php foreach ($colecciones as $key){ if(!empty($key['id_coleccion'])){ if( ($data['id_mercancia'] == $key['id_producto']) && ($key['tipo_inventario_coleccion']=="Mercancia") ){ ?> value="<?=$key['cantidad_productos'] ?>" readonly="" <?php }else{ ?>  <?php }}} ?> >
                                      <input type="hidden" name="elementosid[]" value="m<?=$data['id_mercancia'] ?>">
                                    </td>
                                    <td style="width:10%">
                                      <input type="checkbox" class="cheking check<?=$data['id_mercancia'] ?>" value="m<?=$data['id_mercancia'] ?>" name="cheking[]" <?php foreach ($colecciones as $key){if(!empty($key['id_coleccion'])){ if( ($data['id_mercancia'] == $key['id_producto']) && ($key['tipo_inventario_coleccion']=="Mercancia") ){ echo "checked=''"; }}} ?>>
                          ...
                          <div class="form-group col-sm-6">
                             ...
                             <div class="input-group">
                              ...
                              <input type="number" step="0.1" class="form-control" min="0" id="precioInn" name="precioInn" maxlength="30" placeholder="Precio de la coleccion"
                                <?php foreach ($pagos_despacho as $pagosD){ if(!empty($pagosD['id_despacho'])){
                                  if($pagosD['tipo_pago_despacho']=="Inicial"){
                                    $valInputtemp = $pagosD['pago_precio_coleccion'];
                                    echo "value='{$valInputtemp}'";
                                } } } ?>
                              >
                             </div>
