<?php 
	if(is_file('config/databas3.php')){
		require_once'config/databas3.php';
	}
	else if(is_file('admin/config/databas3.php')){
		require_once'admin/config/databas3.php';
	}
	else if(is_file('../config/databas3.php')){
		require_once'../config/databas3.php';
	}

	
	class Models3 extends Con3xion{
		...
		public function __construct(){Con3xion::realizarConexion();}

		public function registrar($query, $table, $id){
			if(Con3xion::getEstatusConexion()){
				try{
					$strExe = Con3xion::prepare($query);
					$strExe->execute();
					$this->id = Con3xion::getLastId($table,$id);
