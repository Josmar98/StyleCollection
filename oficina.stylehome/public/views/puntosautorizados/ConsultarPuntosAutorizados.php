          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="datatable" class="table table-bordered table-striped" style="text-align:center;width:100%;">
                ...
                <tr>
                  <th>Nº</th>
                  <?php if($accesoPuntosAutorizadosM || $accesoPuntosAutorizadosE){ ?>
                  <th>---</th>
                  <?php } ?>
                  <th>Líder</th>
                  <th>Ciclo</th>
            ...
            <?php 
              $num = 1;
              foreach ($puntos as $data){ if(!empty($data['id_punto'])){
                $permitido = 0;
                ...
                <tr>
                  ...
                  <?php if($accesoPuntosAutorizadosM || $accesoPuntosAutorizadosE){ ?>
                  <td style="width:20%">
                        <?php if($accesoPuntosAutorizadosM==1){ ?>
                          <?php if($data['concepto']!="1"){ ?>
                            <button class="btn modificarBtn" style="border:0;background:none;color:#04a7c9" value="?<?=$menu; ?>&route=<?=$url; ?>&action=Modificar&id=<?php echo $data['id_punto'] ?>">
                              <span class="fa fa-wrench"></span>
                            </button>
                          <?php } ?>
                        <?php } ?>
                        <?php if($accesoPuntosAutorizadosE){ ?>
                          <?php if($data['concepto']!="1"){ ?>
                            <button class="btn eliminarBtn" style="border:0;background:none;color:red" value="?<?=$menu; ?>&route=<?=$url; ?>&id=<?php echo $data['id_punto'] ?>&permission=1">
                              <span class="fa fa-trash"></span>
                            </button>
                          <?php } ?>
                        <?php } ?>
                        <?php if($accesoPuntosAutorizadosM==1){ ?>
                          <?php if($data['estado_puntos']==0){ ?>
                          <button class="btn DesbloquearBtn" style="border:0;background:none;color:<?=$color_btn_sweetalert; ?>" value="?<?=$menu; ?>&route=<?=$url; ?>&id=<?php echo $data['id_punto'] ?>&desbloqueo=1">
                            <span class="fa fa-unlock"></span>
                          </button>
                          <?php } ?>
                          <?php if($data['estado_puntos']==1){ ?>
                          <button class="btn BlquearBtn" style="border:0;background:none;color:purple;" value="?<?=$menu; ?>&route=<?=$url; ?>&id=<?php echo $data['id_punto'] ?>&bloqueo=1">
...
$(document).ready(function(){ 
  ...
  if(response==undefined){
    ...
    if(response == "11"){
      swal.fire({
          type: 'success',
          title: '¡Puntos bloqueados correctamente!',
          confirmButtonColor: "<?=$colorPrimaryAll; ?>",
      }).then(function(){
        var campaing = $(".campaing").val();
        var n = $(".n").val();
    ...
    if(response == "111"){
      swal.fire({
          type: 'success',
          title: '¡Puntos desbloqueados correctamente!',
          confirmButtonColor: "<?=$colorPrimaryAll; ?>",
      }).then(function(){
        var campaing = $(".campaing").val();
        var n = $(".n").val();
  ...
  $(".BlquearBtn").click(function(){
      swal.fire({ 
          title: "¿Desea bloquear la cantidad de puntos?",
          text: "Los puntos serán bloqueados, no estarán disponibles, ¿desea continuar?",
          type: "error",
          showCancelButton: true,
          confirmButtonColor: "<?=$colorPrimaryAll; ?>",
          confirmButtonText: "¡Si!",
  ...
  $(".DesbloquearBtn").click(function(){
      swal.fire({ 
          title: "¿Desea desbloquear la cantidad de puntos?",
          text: "Los puntos volveran a estar disponibles, ¿desea continuar?",
          type: "question",
          showCancelButton: true,
          confirmButtonColor: "<?=$colorPrimaryAll; ?>",
          confirmButtonText: "¡Si!",
